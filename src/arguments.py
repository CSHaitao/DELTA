...
LastEditTime: 2023-12-18 16:42:36
FilePath: /lht/SAILER_english/arguments.py
'''
# import os
...
class DataTrainingArguments:
    ...
    eval_path: str = field(
        default='data.json', metadata={"help": "Path to eval data"}
    )
    test_path: str = field(
        default='data.json', metadata={"help": "Path to test data"}
    )
    train_path: str = field(
        default='data.json', metadata={"help": "Path to train data"}
    )
    model_save_path: str = field(
        default="../model/v0", metadata={"help": "Dir to save or load the ckpt"}
    )
...
class ModelArguments:
    ...
    config_name: Optional[str] = field(
        default='/home/lht/models/bert', metadata={"help": "Pretrained config name or path if not the same as model_name"}
    )
    tokenizer_name: Optional[str] = field(
        default='/home/lht/models/bert', metadata={"help": "Pretrained tokenizer name or path if not the same as model_name"}
    )
    ...
    n_shallow_decoder_layers: int = field(default=1)
    n_deep_decoder_layers: int = field(default=6)
    """
    head mlm
    """
    ...
    translation_coef: Optional[float] = field(
        default=0.5,
        metadata={"help":"the coef for translation"}
    )
    info_nce_coef: Optional[float] = field(
        default=1,
        metadata={"help":"the coef for info nce loss"}
    )
    temperature: Optional[float] = field(
        default=1.0,
        metadata={"help":"the hypermeter used in info nce loss"}
    )
    top_align: Optional[float] = field(
        default=0.8,
        metadata={"help":"the number of fact tokens regarded as the alignment of one reason or judgment token"}
    )



@dataclass
class DELTA_PreTrainingArguments(TrainingArguments):
