...
class DELTA_Collator(DataCollatorForWholeWordMask):
    ...
    def encode_batch_examples_reason(self, examples, mlm_prob: float=0.15):
        encoded_examples = []
        full_input_ids_with_bos = []
        full_input_ids_with_eos = []
        attention_mask_bos = []
        masks = []
        mlm_masks = []
        ...
        for e in examples: 
            ...
            encoded_with_bos = self.tokenizer.encode_plus(
                "[PAD]" + e_trunc,
            ...
            encoded_with_eos = self.tokenizer.encode_plus(
                e_trunc + "[SEP]",
                add_special_tokens=False,
                max_length=self.max_seq_length,
                padding="max_length",
            ...
            masks.append(encoded['attention_mask'])
            encoded_examples.append(encoded['input_ids'])
            full_input_ids_with_bos.append(encoded_with_bos['input_ids'])
            full_input_ids_with_eos.append(encoded_with_eos['input_ids'])
            attention_mask_bos.append(encoded_with_bos['attention_mask'])
        ...
        batch = {
            ...
            "labels": labels,
            "attention_mask": attention_mask,
            "full_input_ids_with_bos": torch.tensor(full_input_ids_with_bos),
            "full_input_ids_with_eos": torch.tensor(full_input_ids_with_eos),
            "attention_mask_bos": torch.tensor(attention_mask_bos)
        }  
        return batch
    ...
    def encode_batch_examples_judgment(self, examples, mlm_prob: float=0.15):
        encoded_examples = []
        full_input_ids_with_bos = []
        full_input_ids_with_eos = []
        attention_mask_bos = []
    ...
    def encode_batch_examples_law(self, examples):
        encoded_examples = []
        full_input_ids_with_bos = []
        full_input_ids_with_eos = []
        attention_mask_bos = []
        ...
        for e in examples:    
            ...
            e_trunc_law_2 = self._truncate(e['judgment'])
            cand_indexes = []
            tokens = [tid for tid in e_trunc_law_2]
            for (i, token) in enumerate(tokens):
                if i > 6:
                    cand_indexes.append(i)
       
            mlm_mask_law_2 = [1 if i in cand_indexes else 0 for i in range(len(tokens))] 
            
            
        
            mlm_mask = [0] + mlm_mask_law + mlm_mask_law_2 + mlm_mask_law
            ...
            encoded = self.tokenizer.encode_plus(
                e_trunc_law + e_trunc_law_2 + e_trunc_law,
                add_special_tokens=True,
                max_length=self.max_seq_length,
                padding="max_length",
                truncation=True,
            ...
            encoded_with_bos = self.tokenizer.encode_plus(
                "[PAD]" + e_trunc_law  + e_trunc_law_2 + e_trunc_law,
                add_special_tokens=False,
                max_length=self.max_seq_length,
                padding="max_length",
                truncation=True,
            ...
            encoded_with_eos = self.tokenizer.encode_plus(
                e_trunc_law + e_trunc_law_2 + e_trunc_law + "[SEP]",
                add_special_tokens=False,
                max_length=self.max_seq_length,
                padding="max_length",
                truncation=True,
    ...
    def process(self, examples, cltypes=None):
        ...
        cause_batch = self.encode_batch_examples_judgment(examples=examples, mlm_prob=self.decoder_mask_ratio)
        ...
        batch['cause_input_ids_with_bos'] = reason_batch['full_input_ids_with_bos']
        batch['cause_input_ids_with_eos'] = reason_batch['full_input_ids_with_eos']
        batch['cause_attention_mask_bos'] = reason_batch['attention_mask_bos']
    ...
    def __call__(self, examples):
        ...
        for text_dict in examples:
            ...
            unpacked.append({'fact': text_dict['Facts'], 'reason':text_dict['Discussion'], 'judgment':text_dict['Conclusion']})
